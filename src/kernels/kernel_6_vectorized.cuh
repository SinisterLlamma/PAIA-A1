 ...
 *
 * Directly based on Simon Boehm's Kernel 6:
 * - Uses 128-bit float4 loads/stores for global memory access
 * - Transposes matrix A into shared memory during loading, so that reading
 *   As during the inner product accesses contiguous memory addresses,
 *   reducing shared memory bank conflicts
 * - Uses float4 vectorized write-back to matrix C
 *
 * Includes a boundary-safe path for non-power-of-2 / odd dimensions.
 */

template <int BM = 128, int BN = 128, int BK = 8, int TM = 8, int TN = 8>
__global__ void sgemm_vectorized_fast(int M, int N, int K, float alpha,
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    // Transpose A while loading into shared memory via float4
    ...
    // Load B into shared memory via float4
  ...
  // Vectorized write-back to C via float4
  for (uint resM = 0; resM < TM; ++resM) {
...
// Fallback kernel with scalar boundary handling for unaligned / odd sizes
template <int BM = 64, int BN = 64, int BK = 8, int TM = 4, int TN = 4>
__global__ void sgemm_vectorized_safe(int M, int N, int K, float alpha,
                                      const float *A, const float *B, float beta,
                                      float *C) {
  ...
  for (int tileK = 0; tileK < K; tileK += BK) {
    ...
    for (int k = 0; k < BK; ++k) {
      ...
      for (int tn = 0; tn < TN; ++tn) {
        regB[tn] = Bs[k * BN + threadCol * TN + tn];
      }
      for (int tm = 0; tm < TM; ++tm) {
        for (int tn = 0; tn < TN; ++tn) {
...
inline void launch_kernel_vectorized(int M, int N, int K, float alpha,
                                     const float *A, const float *B,
                                     float beta, float *C) {
  // Use fast 128-bit vectorized float4 kernel when dimensions are multiples of 128 (or 8)
  if (M % 128 == 0 && N % 128 == 0 && K % 8 == 0 &&
      ((uintptr_t)A % 16 == 0) && ((uintptr_t)B % 16 == 0) && ((uintptr_t)C % 16 == 0)) {
    const int BM = 128, BN = 128, BK = 8, TM = 8, TN = 8;
    dim3 block((BM * BN) / (TM * TN));
    dim3 grid(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    sgemm_vectorized_fast<BM, BN, BK, TM, TN>
        <<<grid, block>>>(M, N, K, alpha, A, B, beta, C);
  } else {
    const int BM = 64, BN = 64, BK = 8, TM = 4, TN = 4;
    dim3 block((BM * BN) / (TM * TN));
    dim3 grid(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    sgemm_vectorized_safe<BM, BN, BK, TM, TN>
        <<<grid, block>>>(M, N, K, alpha, A, B, beta, C);
  }
}
